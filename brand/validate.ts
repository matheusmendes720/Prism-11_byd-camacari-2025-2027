...
function findValidator(): string {
  ...
  throw new Error(
    "validate_palette.js not found. Set ATLAS_VALIDATOR=/path/to/validate_palette.js, " +
    "or copy validate_palette.js to ./scripts/ or ./vendor/scripts/.",
  );
}

type Mode = "light" | "dark";
type Result = { ok: boolean; report: unknown[] };

function runValidator(hexCsv: string, mode: Mode, surfaceHex: string, extraArgs: string[] = []): Result {
...
function categoricalHexList(mode: Mode): string[] {
  return ([1, 2, 3, 4, 5, 6, 7, 8] as const).map(s => seriesHex(s, mode));
}

function checkCategorical(mode: Mode): Result {
  const hexCsv = categoricalHexList(mode).join(",");
  return runValidator(hexCsv, mode, surface.chart[mode]);
}

function checkRamp(rampName: keyof typeof ramp, mode: Mode): Result {
...
function main(): void {
  ...
  const modes: Mode[] = onlyMode ? [onlyMode] : ["light", "dark"];
  let allOk = true;
  ...
  for (const mode of modes) {
    ...
    if (!cat.ok) allOk = false;

    for (const name of ["yellow", "red", "cyan"] as const) {
      console.log(`\nRamp ${name}:`);
